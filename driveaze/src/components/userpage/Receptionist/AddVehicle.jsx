...
function AddVehicle() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await UserService.addCustomerVehicle(updatedVehicleData, token);
              ...
              {/* <div>
                ...
                >
                  ...
                  {vehicleBrands.map((brand) => (
                    <option key={brand.brandId} value={brand.vehicleBrand}>
                      {brand.brandName}
                    </option>
                  ))}
                </select>
                ...
                >
                  ...
                  {vehicleModels.length > 0 ? (
                    vehicleModels.map((model) => (
                      <option key={model.modelId} value={model.vehicleModel}>
                        {model.modelName}
                      </option>
                    ))
                  ) : (
